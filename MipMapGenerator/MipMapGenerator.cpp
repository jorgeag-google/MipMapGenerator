...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < levels_to_generate; ++i) {
        ...
        mip_maps[i].height = mip_maps[i - 1].height > 1 ? mip_maps[i - 1].height / 2 : 1;
        mip_maps[i].level  = mip_maps[i - 1].level + 1;
        mip_maps[i].desired_channels = mip_maps[i - 1].desired_channels;
        mip_maps[i].original_channels = mip_maps[i - 1].original_channels;
